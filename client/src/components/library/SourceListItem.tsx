...
function SourceListItem(props: SourceListItemProps) {
  ...
  const onSourceIconClick = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    //   } catch (e) {
    //     dispatch(systemMessage('The source ' + source?.url + " isn't in your Library"))
    //   }
    // }
  }
  ...
  const onClickBlacklist = (e: MouseEvent) => {
    if (e.shiftKey) {
      clearBlacklist(source?.url as string)
    } else {
  ...
  return (
    <div
      ...
      className={cx(
        ...
        tutorial?.current === SDT.source && classes.highlight,
        tutorial?.current && classes.disable
      )}
    >
      <ListItem
        secondaryAction={
          props.isEditing !== props.source && (
            ...
            >
              ...
              {sourceType === ST.video && source?.duration && (
                <Chip
                  className={classes.countChip}
                  label={getTimestamp(source?.duration)}
                  color="secondary"
                  size="small"
                />
              )}
              {sourceType === ST.video && !!source?.resolution && (
                <Chip
                  className={classes.countChip}
              ...
              {(source?.count ?? 0) > 0 && sourceType !== ST.video && (
                <Chip
                  className={cx(
                    classes.countChip,
                    tutorial?.current === SDT.sourceCount && classes.highlight
                  )}
          ...
          >
            ...
            >
              <Fab
                ...
                className={cx(
                  classes.avatar,
                  source?.marked && classes.markedSource,
                  tutorial?.current === SDT.sourceAvatar && classes.highlight
                )}
              >
                <SourceIcon
                  type={source?.type}
