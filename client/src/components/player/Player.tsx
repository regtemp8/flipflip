...
//   const { isWin32, pathSep } = useAppSelector(selectConstants())
  ...
  const isEmpty = false //useAppSelector(selectPlayerIsEmpty(props.uuid))

  const watermark = undefined //useAppSelector(selectAppConfigGeneralSettingsWatermarkSettings(false))
  ...
  const nextSceneAllImages = false //useAppSelector(selectSceneNextSceneAllImages(sceneID))
  const nextSceneTime = 0 //useAppSelector(selectSceneNextSceneTime(sceneID))
  const persistAudio = false //useAppSelector(selectScenePersistAudio(sceneID))
  const audioEnabled = false //useAppSelector(selectSceneAudioEnabled(sceneID))
  const persistText = false //useAppSelector(selectScenePersistText(sceneID))
  const textEnabled = false //useAppSelector(selectSceneTextEnabled(sceneID))
  const strobe = false //useAppSelector(selectSceneStrobe(sceneID))
  const strobeLayer = undefined //useAppSelector(selectSceneStrobeLayer(sceneID))
  const scriptPlaylists: number[] = [] //useAppSelector(selectSceneScriptPlaylists(sceneID))
