...
export default function SourceScraper(props: SourceScraperProps) {
  ...
  const opacitySelector = isOverlay
    ? selectPlayerOverlayOpacity(props.overlayIndex as number, props.uuid)
    : (state: RootState) => props.opacity
  const opacity = useAppSelector(opacitySelector)
  ...
  const onCloseDialog = () => {
    dispatch(
      setPlayerCaptcha({
        uuid: props.uuid,
        value: { overlayIndex: props.overlayIndex }
      })
    )
    setLoad(false)
  }
