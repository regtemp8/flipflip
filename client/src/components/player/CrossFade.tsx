...
interface CrossFadeTransitionProps extends CSSProperties {
  volume?: number
}

interface CrossFadeTransitionComponentProps {
  key: React.Key | null | undefined
  volume?: number
  style: CSSProperties
}

function CrossFadeTransitionComponent(
  props: PropsWithChildren<CrossFadeTransitionComponentProps>
) {
...
export default function CrossFade(props: PropsWithChildren<CrossFadeProps>) {
  ...
  const fadeTransitions: UseTransitionResult<
    ...
    ForwardedProps<CrossFadeTransitionProps>
  >[] = useTransition<
    HTMLImageElement | HTMLVideoElement | HTMLIFrameElement,
  ...
  const Component = animated(CrossFadeTransitionComponent)
  return (
    <React.Fragment>
