...
export default class ScenePlaylistPlayer {
  ...
  public next(): ScenePlaylistItem | undefined {
    ...
    const sceneId =
      sceneIds.length === 1 ? sceneIds[0] : getRandomListItem(sceneIds)
  ...
  public static async create(viewId: number): Promise<ScenePlaylistPlayer> {
    ...
    const { id, repeat, shuffle } = (await findPlaylistByDisplayView(
      viewId
    )) as Playlist
    ...
    for (const playlistItem of playlistItems) {
      ...
      if (item == null) {
        item = {
          ...
          sceneIds:
            playlistItem.sceneId != null ? [playlistItem.sceneId] : undefined
        }
      } else {
    ...
    return new ScenePlaylistPlayer(
      allSceneIds,
      repeat,
      toBoolean(shuffle),
