...
import { CacheSettings, ContentSource, getSourceType, randomizeList, SOF, ST, Scene, RemoteSettings, ScraperHelpers, PlayerCaptcha, ScraperProgress } from "flipflip-common"
import { toCacheSettings, toRemoteSettings, toScene, toContentSource } from "../db/mappers"
import { findSceneById } from "../db/SceneRepository"
import { Scene as SceneRow, User } from "../db/types/generated"
import { findContentSources, findContentSourceTagIds, updateContentSourceCount } from "../db/ContentSourceRepository"
...
export default class SourceScraper {
    ...
    public static async create(sceneId: number, user: User): Promise<SourceScraper> {
        const scene = toScene(await findSceneById(sceneId) as SceneRow)

        const sources: ContentSource[] = []
        const sourceRows = await findContentSources(sceneId)
        for (const row of sourceRows) {
            const clips: number[] = scene.playVideoClips ? await findContentSourceClipIds(user.id as number, row.id as number) : []
            const tags = await findContentSourceTagIds(user.id as number, row.id as number)
            if (toBoolean(row.localDirOfSources) && getSourceType(row.url) === ST.local) {
                try {
    ...
    private constructor(scene: Scene, caching: CacheSettings, remoteSettings: RemoteSettings, sceneSources: ContentSource[]) {
        this.scene = scene
    ...
    private async sourceLoop() {
        ...
        if (object?.source) {
            ...
            if (object?.data && object?.allURLs) {
                ...
                await updateContentSourceCount(source.url, helpers?.count ?? 0, helpers?.next == null)
            }
        }
        ...
        if (current < total) {
            ...
            setTimeout(() => { this.sourceLoop() }, timeout)
        } else {
            setTimeout(() => {
                this.promiseLoop()
            }, 200)
    ...
    private async promiseLoop() {
        ...
        if (object?.source) {
            ...
            const timeout = object?.timeout ?? 1000
            setTimeout(() => { this.promiseLoop() }, timeout)
        }
    }
}