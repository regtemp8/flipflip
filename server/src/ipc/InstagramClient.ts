import { type ChallengeStateResponse, IgApiClient, type SavedFeedResponseMedia, type UserFeedResponseItemsItem, type AccountFollowingFeedResponseUsersItem } from 'instagram-private-api'
...
export default class InstagramClient {
  ...
  async onRequestUserFeedItems (username: string): Promise<InstagramItems<UserFeedResponseItemsItem>> {
    const client = this.getClient()
  ...
  async onRequestGetMore (
    ...
    id: number | undefined,
    feedSession: string
  ): Promise<InstagramItems<SavedFeedResponseMedia | UserFeedResponseItemsItem> | undefined> {
    const client = this.getClient()
  ...
  async onRequestFollowingFeed (userId: number): Promise<InstagramItems<AccountFollowingFeedResponseUsersItem>> {
    const client = this.getClient()
  ...
  async onRequestGetMoreFollowingFeed (
    ...
    userId: number,
    feedSession: string
  ): Promise<InstagramItems<AccountFollowingFeedResponseUsersItem> | undefined> {
    const client = this.getClient()
    await client.state.deserializeCookieJar(JSON.parse(session))
    const followingFeed = client.feed.accountFollowing(userId)
