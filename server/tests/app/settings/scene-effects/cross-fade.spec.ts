...
test('Cross-fade effect', async ({ page }) => {
    ...
    await expect(page.getByLabel('Cross-Fade Audio', { exact: true })).not.toBeVisible()
    ...
    await expect(page.getByLabel('Cross-Fade Audio', { exact: true })).toBeVisible()
    await expect(page.getByLabel('Cross-Fade Audio', { exact: true })).not.toBeChecked()
    ...
    await expect(page.getByLabel('Cross-Fade Audio', { exact: true })).toBeChecked()
...
test('Wave cross-fade timing', async ({ page }) => {
    ...
    const input = await container.locator('.MuiCollapse-entered .MuiTextField-root input[aria-labelledby="fade-sin-rate-slider"]')
