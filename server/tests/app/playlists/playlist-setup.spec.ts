...
test('Add scene playlist items', async ({ page }) => {
  ...
  await page.locator('#scene-playlist-item-scene-select').click()
  await expect(
    page.getByRole('option', { name: 'Test' }).getByTestId('ErrorOutlineIcon')
  ).toBeVisible()
  await page.getByText('CancelSave').click()
  await page.getByRole('spinbutton', { name: 'Play for' }).click()
  await page.getByRole('spinbutton', { name: 'Play for' }).fill('400000')
  await expect(page.getByRole('spinbutton', { name: 'Play for' })).toHaveValue(
    '400000'
  )
  await page.getByRole('button', { name: 'Save' }).click()
  ...
  await page
    .locator(
      '.MuiButtonBase-root.MuiFab-root.MuiFab-circular.MuiFab-sizeLarge.MuiFab-default'
    )
    .click()
  ...
  await expect(
    page.getByRole('checkbox', { name: 'Play After All Images' })
  ).toBeChecked()
  await expect(
    page.getByRole('spinbutton', { name: 'Play for' })
  ).not.toBeVisible()
...
test('Move scene playlist item', async ({ page }) => {
  ...
  await dragAndDrop(
    page,
    '#scene-playlist-items li:last-child',
    '#scene-playlist-items li:first-child'
  )
  ...
  await dragAndDrop(
    page,
    '#scene-playlist-items li:nth-child(2)',
    '#scene-playlist-items li:first-child'
  )
  await expect(listItems.nth(0)).toHaveText('None')
