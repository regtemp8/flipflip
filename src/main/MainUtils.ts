...
import { getSaveDir } from './main'

export const isWin32 = process.platform === 'win32'
export const isMacOSX = process.platform === 'darwin'
export const pathSep = path.sep

export function cachePath (baseDir: string, source?: string, typeDir?: string) {
  if (baseDir !== '') {
...
export function getFilesRecursively (fsPath: string): string[] {
  const isDirectory = (fsPath: string) => fs.statSync(fsPath).isDirectory()
  const getDirectories = (fsPath: string) =>
    fs
      .readdirSync(fsPath)
      .map((name) => path.join(fsPath, name))
      .filter(isDirectory)

  const isFile = (fsPath: string) => fs.statSync(fsPath).isFile()
  const getFiles = (fsPath: string) =>
    fs
      .readdirSync(fsPath)
      .map((name) => path.join(fsPath, name))
      .filter(isFile)
