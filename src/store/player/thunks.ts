...
import { PT } from '../../renderer/data/const'
...
import { type PlayersState, setPlayersState, setPlayerStates, PlayerOverlayState, setPlayerHasStarted, PlayerState, PlayerUpdate } from './slice'
import {
...
function createPlayerState (
  ...
  const overlays = scene.overlayEnabled && scene.overlays
    ? scene.overlays.map((overlayID) => {
      ...
      if (isGrid) {
        sceneID = convertSceneIDToGridID(overlay.sceneID)
...
export function nextScene (uuid: string = 'root') {
  return function nextSceneThunk (
    ...
    if (scene != null) {
      ...
      const updates: PlayerUpdate<PlayerState>[] = Object.keys(newPlayerState)
        .map((uuid) => {
...
export function playScene (id: number) {
  return (dispatch: AppDispatch, getState: () => RootState): void => {
    ...
    const tutorial = state.app.config.tutorials.player == null ? PT.welcome : null
    const uuid = 'root'
    const playersState: PlayersState = {}
    createPlayerState(uuid, scene, playersState, state)
    dispatch(setPlayersState(playersState))
...
export function setPlayerHasStartedRecursive (uuid: string, hasStarted: boolean) {
  return (dispatch: AppDispatch, getState: () => RootState): void => {
    const state = getState()
