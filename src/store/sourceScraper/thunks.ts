...
import LibrarySource, { newLibrarySource } from '../../storage/LibrarySource'
...
import { ST, SOF } from '../../renderer/data/const'
import {
  ScrapedSources,
...
function getPlayerSceneID(player: PlayerState, isNextSceneID: boolean, overlayIndex?: number) {
  if(isNextSceneID) {
  ...
  if (playerStopped) {
    ...
    if (newPlayerUUID == null) {
      newPlayerUUID = Object.keys(state.players).find(
        (uuid) => state.players[uuid].overlays.find((s) => s.sceneID === sceneID) != null
      )
    }
    if (newPlayerUUID == null) {
      newPlayerUUID = Object.keys(state.players).find((uuid) => {
        const p = state.players[uuid]
        return ( // only return true for next scene if current scene is loaded
          p.nextSceneID === sceneID && state.sourceScraper[p.sceneID]?.completed
        )
      })
    }

    dispatch(setSourceScraperSourcesWorker({ id: sceneID, value: undefined }))
    if (newPlayerUUID != null) {
...
function promiseLoop(
  ...
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (isPlayerStopped(dispatch, state, playerUUID, sceneID, isNextSceneID, overlayIndex)) return
    ...
    const receiveMessage = (message: any) => {
      ...
      if (object?.source) {
        if (object?.data) {
          ...
          sources.allURLs = convertMapToRecord(object.allURLs)
          sources.allPosts = convertMapToRecord(object.allPosts)

          // Add the next promise to the queue
          const { source, helpers } = object
          if (helpers.next != null) {
            sources.promiseQueue = sources.promiseQueue.concat([
              { source, helpers }
            ])
          }

          dispatch(setSourceScraperSources({ id: sceneID, value: sources }))
          dispatch(
            setCount(
              source.url,
              helpers.count,
    ...
    const workerListener = receiveMessage.bind(this)
    if (config.generalSettings.prioritizePerformance) {
      window.flipflip.events.onWorkerResponse(workerUUID, workerListener)
      scrapeFiles(
        workerListener,
        convertRecordToMap(sources.allURLs),
...
function sourceLoop(
  ...
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    const {total, current} = scraperState.progress
    if (current === total) {
      dispatch(setSourceScraperSourcesWorker({ id: sceneID, value: undefined }))
      if(!isNextSceneID) {
    ...
    if(!isNextSceneID) {
      ...
      dispatch(setSourceScraperProgress({id: sceneID, value: {total, current: (current + 1), message}}))
    }
  ...
  return async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    for (const source of sources) {
      if (source.dirOfSources && getSourceType(source.url) === ST.local) {
        try {
          const directories = await window.flipflip.api.getDirectories(source.url)
          for (const d of directories) {
            sceneSources.push(newLibrarySource({ url: source.url + pathSep + d }))
          }
        } catch (e) {
...
function shouldContinueScrape(
  ...
  return scrapedSources.progress && scrapedSources.progress.current < scrapedSources.progress.total
}
...
function shouldScrapePromises(
  ...
  const scrapedSources = sourceScraperState[sceneID]
  return scrapedSources.promiseQueue.length > 0 && scrapedSources.workerUUID == null
}
...
function scheduleScrape(
  ...
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    if (shouldStartScrape(sceneID, state.sourceScraper)) {
      ...
      setTimeout(() => {
        dispatch(promiseLoop(playerUUID, sceneID, isNextSceneID, overlayIndex))
        dispatch(scheduleScrape(playerUUID, sceneID, isNextSceneID, workerUUID, overlayIndex))
      }, 200)
      scheduled = true
    }
...
export function scrapeSources(playerUUID: string) {
  return (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    const scheduled = dispatch(scheduleScrape(playerUUID, sceneID, false, workerUUID))
    if(!scheduled) {
...
async function cachePath(
  ...
  return config.caching.enabled &&
    (await window.flipflip.api.pathExists(cachePath))
    ? cachePath
    : null
}
...
async function scrapeFiles(
  ...
  if (sourceType === ST.local) {
    ...
    if (returnPromise) {
      ...
      window.flipflip.api.loadInWorker('loadRemoteImageURLList', [
        allURLs,
        allPosts,
        config,
        source,
    ...
    if (helpers.next === -1) {
      helpers.next = 0
      const cachePath = await getCachePath(config.caching.directory, source.url)
      if (
        config.caching.enabled &&
        (await window.flipflip.api.hasFiles(cachePath))
      ) {
...
const loadLocalDirectory = async (
  ...
  try {
    data = url
      ? await window.flipflip.api.recursiveReaddir(
          url,
          blacklist,
          source.blacklist,
          filter,
...
export const loadVideo = async (
  ...
  const ifExists = async (url: string) => {
    if (!url.startsWith('http')) {
      url = await window.flipflip.api.getFileUrl(url)
    }
    helpers.count = 1
  ...
  if (url.startsWith('http')) {
    ...
    const exists = await window.flipflip.api.pathExists(url)
    if (exists) {
      ifExists(url)
    } else {
      missingVideo()
...
const loadInWorker = (event: string) => {
  return (
    ...
    window.flipflip.api.loadInWorker(event, [
      allURLs,
      allPosts,
      config,
      source,
