...
import { selectSceneGroups, selectSceneGroupScenes, selectSceneGroupGrids } from '../sceneGroup/selectors'
...
import { convertGridIDToSceneID } from '../../renderer/data/utils'
...
const selectGroupedScenes = (type: string) => {
  return createSelector(
    ...
    (type, groups) => groups.filter((g) => g.type === type).flatMap((g) => g.scenes)
  )
}
    ...
    [selectGroupedScenes(SG.scene), selectScenes(), getScenePickerFilters], 
      (groupedScenes, scenes, filters) => {
        return scenes.filter((s) =>         
          ...
          !isGeneratorScene(s) &&
          isDisplayScene(s, filters)).map((s) => s.id)
    }
  )
}
    ...
    [selectGroupedScenes(SG.grid), selectSceneGrids(), getScenePickerFilters], 
      (groupedScenes, grids, filters) => {
        return grids.filter((s) =>         
          ...
          isDisplayGrid(s, filters)).map((s) => s.id)
    }
  )
}
...
export const selectScenePickerSceneGroups = () => {
  return createSelector(
    [selectSceneGroups()], 
    (groups) => groups.filter((s) => s.type === SG.scene).map((s) => s.id)
  )
}
...
export const selectScenePickerGeneratorGroups = () => {
  return createSelector(
    [selectSceneGroups()], 
    (groups) => groups.filter((s) => s.type === SG.generator).map((s) => s.id)
  )
}
...
export const selectScenePickerGridGroups = () => {
   return createSelector(
    [selectSceneGroups()], 
    (groups) => groups.filter((s) => s.type === SG.grid).map((s) => s.id)
  )
}
...
const selectScenePickerSceneGroupItems = (groupID: number) => {
  return createSelector(
    ...
    (scenes, filters) => scenes?.filter((s) => !isGeneratorScene(s) && isDisplayScene(s, filters)).map((s) => s.id)
  )
}
...
const selectScenePickerGridGroupItems = (groupID: number) => {
  return createSelector(
    ...
    (grids, filters) => grids?.filter((s) => isDisplayGrid(s, filters)).map((s) => s.id) 
  )
}
...
const isDisplayScene = (scene: Scene, filters: string[]) => {
  return (
    filters.length === 0 ||
    nameMatchesFilter(scene.name, filters)
  )
}

const isDisplayGrid = (grid: SceneGrid, filters: string[]) => {
  return filters.length === 0 || (grid.name !== undefined && nameMatchesFilter(grid.name, filters))
}
