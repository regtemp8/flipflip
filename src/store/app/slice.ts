...
const appSlice = createSlice({
  ...
  reducers: {
    ...
    setConfigDisplaySettingsAlwaysOnTop: (
      ...
      state.config.displaySettings.alwaysOnTop = action.payload
    },
    setConfigDisplaySettingsShowMenu: (
      state,
      action: PayloadAction<boolean>
    ) => {
      state.config.displaySettings.showMenu = action.payload
    },
    ...
    setThemePalettePrimary: (state, action: PayloadAction<Record<string, string>>) => {
      state.theme.palette.primary = action.payload
    },
    setThemePaletteSecondary: (state, action: PayloadAction<Record<string, string>>) => {
    ...
    addScriptSingle: (state, action: PayloadAction<void>) => {
      state.route.push(newRoute({ kind: 'scripts', value: null }))
      state.specialMode = SP.selectSingle
      state.scriptSelected = []
    },
    openLibraryImport: (state, action: PayloadAction<void>) => {
      state.route.push(newRoute({ kind: 'library', value: null }))
      state.specialMode = SP.select
    ...
    addToPlaylist: (state, action: PayloadAction<void>) => {
      state.specialMode =
        state.specialMode !== SP.addToPlaylist ? SP.addToPlaylist : null
      state.audioOpenTab = 3
    },
    batchClip: (state, action: PayloadAction<void>) => {
      state.specialMode =
        state.specialMode !== SP.batchClip ? SP.batchClip : null
      state.audioOpenTab = 3
    },
    batchTag: (state, action: PayloadAction<void>) => {
      state.specialMode = state.specialMode !== SP.batchTag ? SP.batchTag : null
      state.audioOpenTab = 3
    },
    batchEdit: (state, action: PayloadAction<void>) => {
      state.specialMode =
        state.specialMode !== SP.batchEdit ? SP.batchEdit : null
      state.audioOpenTab = 3
    },
    manageTags: (state, action: PayloadAction<void>) => {
      state.route.push(newRoute({ kind: 'tags', value: null }))
    },
