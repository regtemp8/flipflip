...
export const selectSceneGridCellMatchesSceneCopy = (
  ...
  return (state: RootState) => {
    return getSceneGrid(state.sceneGrid, id).grid.find((r) =>
      r.find(
        (c) => c.sceneCopy[0] === sceneCopy[0] && c.sceneCopy[1] === sceneCopy[1]
      )
    )
  }
}
...
export const getSceneGridEntries = (state: RootState) => state.sceneGrid.entries

export const selectSceneGrids = () => {
  return createSelector(
    [getAppGrids, getSceneGridEntries], 
    (ids, entries) => ids.map((id => entries[id] as SceneGrid))
  )
}