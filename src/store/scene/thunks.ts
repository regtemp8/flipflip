...
import { addLibrarySources, setLibrarySource, setLibrarySources } from '../librarySource/slice'
import {
  areWeightsValid,
  filterSource,
  reduceList,
...
import { getSourceUrl } from '../../renderer/data/actions'
...
import { getScene, selectVideoClipperScene } from './selectors'
...
export function generateScenes (
  ...
  return (dispatch: AppDispatch, getState: () => RootState): void => {
    ...
    for (const scene of generateScenes) {
      ...
      // Sources to require
      let reqAdvSources: LibrarySource[] = null
      // Sources to exclude
      let excAdvSources: LibrarySource[] = null
        ...
        if (ruleAllSearches.length + ruleOrSearches.length + ruleNoneSearches.length === wg.rules.length) {
          ...
          for (const sourceID of state.app.library) {
            ...
            if (!addedClip) {
              ...
              for (const as of ruleAllSearches) {
                if (!filterSource(as, s, null, state.tag.entries)) {
                  b = true
                  break
                }
              }
              ...
              let anyRos = ruleOrSearches.length === 0
              for (const os of ruleOrSearches) {
                if (filterSource(os, s, null, state.tag.entries)) {
                  anyRos = true
                  break
                }
              }
              ...
              for (const ns of ruleNoneSearches) {
                if (filterSource(ns, s, null, state.tag.entries)) {
                  b = true
                  break
                }
              }
              ...
              for (const sourceID of state.app.library) {
                ...
                if (
                  ...
                  for (const clipID of s.clips) {
                    ...
                    if (!filterSource(rule.search, s, c, state.tag.entries)) {
                      invalidClips.push(c.id)
                      continue
                    }
                ...
                if (!addedClip) {
                  ...
                  if (!filterSource(rule.search, s, null, state.tag.entries)) {
                    continue
                  }
          ...
          for (const sourceID of state.app.library) {
            ...
            if (
              ...
              for (const clipID of s.clips) {
                ...
                // Filter out clips which don't match this search
                if (!filterSource(wg.search, s, c, state.tag.entries)) {
                  invalidClips.push(c.id)
                  continue
                }
            ...
            if (!addedClip) {
              ...
              // Filter out sources which don't match this search
              if (!filterSource(wg.search, s, null, state.tag.entries)) {
                continue
              }
...
export function addSource (type: string, sceneID?: number, ...args: any[]) {
  return async (dispatch: AppDispatch, getState: () => RootState): Promise<void> => {
    ...
    if (type === 'tutorial') {
      ...
      dispatch(setSceneAddSource({ id: sceneID, value: source.id }))
      ...
      switch (type) {
        case AF.url:
          ...
          break
        case AF.list:
          newSources = Array.from(args[0].trim().split("\n")).filter((s: string) => s.length > 0) as Array<string>;
          break
        case AF.directory:
          newSources = await window.flipflip.api.openDirectories()
          break
        case AF.videos:
          newSources = await window.flipflip.api.openVideos()
          break
        case AF.videoDir:
          newSources = await window.flipflip.api.loadVideoSources()
          break
        case GT.local:
      ...
      if (sceneID != null) {
        ...
        dispatch(setSceneAddSources({ id: sceneID, value: sources.map((s) => s.id) }))
        handleArgs(sceneID)
      } else {
...
export function exportScene (sceneID: number) {
  return (dispatch: AppDispatch, getState: () => RootState): void => {
    const state = getState()
    const scenesToExport = Array<SceneStorage.default>()
    const gridsToExport = Array<SceneGridStorage.default>()
    ...
    if (sceneCopy.overlayEnabled) {
      for (const o of sceneCopy.overlays) {
        ...
        if (isSceneIDAGridID(o.sceneID)) {
          ...
          if (overlay !== undefined && !scenesToExport.find((s) => s.id === o.sceneID)) {
    ...
    window.flipflip.api.saveJsonFile(fileName, sceneExport)
  }
}
...
export function setVideoClipperSceneVideoVolume(videoVolume: number) {
  return (dispatch: AppDispatch, getState: () => RootState): void => {
    const scene = selectVideoClipperScene()(getState())
