...
export default function CodeMirror (props: CodeMirrorProps) {
  ...
  const _sendUpdate = useRef<number>(null)
  ...
  useEffect(() => {
    ...
    return () => {
      if (props.addHack) {
        props.addHack.listener = null
      }
      if (props.overwriteHack) {
        props.overwriteHack.listener = null
      }
    }
  }, [])
  ...
  const onBeforeChangeScript = (
    ...
    if (scriptText !== value) {
      onUpdateScript(value, editor, true)
    }
  }
  ...
  const onUpdateScript = (
    ...
    _sendUpdate.current = window.setTimeout(
      props.onUpdateScript.bind(this, scriptText, changed),
      500
    )
  }
  ...
  return (
    <CodeMirrorComp.Controlled
      ...
      onBeforeChange={onBeforeChangeScript.bind(this)}
      onCursorActivity={onCursorActivity.bind(this)}
