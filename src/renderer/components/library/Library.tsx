...
function Library(props: WithStyles<typeof styles>) {
  ...
  const onFinishMove = async () => {
    for (const source of library) {
      if (source.offline) {
        ...
        try {
          files = await window.flipflip.api.readdir(cachePath)
        } catch (err) {
          error = err
        }
        ...
        if (!!error || files.length === 0) {
          ...
          await window.flipflip.api.move(cachePath, localPath)
          dispatch(
  ...
  const onFinishImportLibrary = async () => {
    if (importFile.startsWith('http')) {
      ...
      const text = await window.flipflip.api.readTextFile(importFile)
