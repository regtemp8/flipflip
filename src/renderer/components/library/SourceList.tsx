...
import { RF, RT, SDT, ST, SDGT } from '../../data/const'
...
function BlacklistDialog (props: BlacklistDialogProps) {
  ...
  const onChangeBlacklist = (e: MouseEvent) => {
    setBlacklistToEdit((e.currentTarget as HTMLInputElement).value)
  }
    ...
    >
      <DialogContent>
        ...
        <TextField
          ...
          onChange={onChangeBlacklist.bind(this)}
        />
      </DialogContent>
      <DialogActions>
        <Button
...
function SourceOptionsDialog(props: SourceOptionsDialogProps) {
  ...
  const onOpenSubtitleFile = async () => {
    const subtitleFile = await window.flipflip.api.openSubtitleFile()
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <Tooltip disableInteractive title="Open File">
                      <IconButton
                        onClick={onOpenSubtitleFile.bind(this)}
                        size="large"
                      >
                        <FolderIcon />
                      </IconButton>
...
function SourceList(props: SourceListProps) {
  ...
  const yOffset = props.isLibrary ? useAppSelector(selectAppLibraryYOffset()) : 0
  const firstSourceURL = useAppSelector(selectLibrarySourceURL(props.sources.length > 0 ? props.sources[0] : -1))
  ...
  const onFinishDelete = async (sourceID: number, sourceURL: string) => {
    ...
    if (fileType === ST.local) {
      window.flipflip.api.rimrafSync(sourceURL)
    } else if (
      fileType === ST.video ||
      fileType === ST.playlist ||
      fileType === ST.list
    ) {
      await window.flipflip.api.unlink(sourceURL)
    }
    onRemove(sourceID)
    onCloseDeleteDialog()
  }
  ...
  const onFinishClean = () => {
    window.flipflip.api.rimrafSync(cachePath as string)
    onCloseClean()
  }
    ...
    ({ value }: { value: { index: number; style: any; data: any[] } }) => {
      ...
      return (
        <SourceListItem
          ...
          onClean={onClean.bind(this)}
          onDelete={onDelete.bind(this)}
          onEditBlacklist={onEditBlacklist.bind(this)}
          onEndEdit={onEndEdit.bind(this)}
          onOpenClipMenu={onOpenClipMenu.bind(this)}
          onOpenWeightMenu={onOpenWeightMenu.bind(this)}
  ...
  return (
    <React.Fragment>
      ...
      {cachePath != null && (
        <Dialog
          open={true}
          onClose={onCloseClean.bind(this)}
        ...
        >
          <DialogContent>
            <DialogContentText id="clean-cache-description">
              ...
              <Link
                ...
                href="#"
                onClick={openDirectory.bind(this, cachePath)}
                underline="hover"
              >
                {cachePath}
              </Link>{' '}
          ...
          <DialogActions>
            <Button onClick={onCloseClean.bind(this)} color="secondary">
              Cancel
            </Button>
            <Button onClick={onFinishClean.bind(this)} color="primary">
              OK
            </Button>
          </DialogActions>
        </Dialog>
