...
import { convertFromEpoch } from '../../../renderer/data/utils'
...
export default function ErrorCard(props: ErrorCardProps) {
  ...
  const onSubmitIssue = () => {
    ...
    window.flipflip.api.reportError(title, body)
  }

  const reset = async () => {
    await window.flipflip.api.rimrafSync(savePath)
    window.flipflip.api.reloadWindow()
  }
  ...
  const getBackups = async () => {
    const backups = await window.flipflip.api.getBackups()
    setBackupCheck(true)
    setBackups(backups)
