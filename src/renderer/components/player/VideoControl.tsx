...
function VideoControl (props: VideoControlProps) {
  const volume = useAppSelector(selectClipVolume(props.clipID))
  ...
  const onChangePosition = (e: MouseEvent, position: number) => {
    props.video.currentTime = position
    if (props.video.paused) {
      triggerUpdate()
    }
  }

  const onChangeVolume = (e: MouseEvent, volume: number) => {
    if (volume > 100) {
      volume = 100
    }
    if (volume < 0) {
  ...
  const onChangeSpeed = (e: MouseEvent, speed: number) => {
    props.onChangeSpeed(speed)
    if (props.video) {
      props.video.playbackRate = speed / 10
      triggerUpdate()
    }
  }
  ...
  const onBack = () => {
    ...
    onChangePosition(null, position)
  }
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        if (e.ctrlKey) {
          e.preventDefault()
          onChangeVolume(null, props.video.volume * 100 + 5)
        }
        break
      case 'ArrowDown':
        if (e.ctrlKey) {
          ...
          <Grid item xs={12} style={{ textAlign: 'center' }}>
            {props.nextTrack && showSpeed && (
              <Tooltip disableInteractive title="Show Volume Controls">
                <IconButton onClick={onSwapSlider.bind(this)} size="large">
                  <VolumeUpIcon />
                </IconButton>
              </Tooltip>
            )}
            ...
            {props.nextTrack && (
              <Tooltip disableInteractive title="Next Track">
                <IconButton onClick={props.nextTrack.bind(this)} size="large">
                  <SkipNextIcon />
                </IconButton>
              </Tooltip>
            )}
            ...
            {showSpeed && (
              <React.Fragment>
                ...
                <Slider
                  ...
                  onChangeCommitted={onChangeSpeed.bind(this)}
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs>
                  <Slider
                    ...
                    onChange={onChangeVolume.bind(this)}
                    marks={
