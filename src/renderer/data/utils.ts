...
export async function extractMusicMetadata (
  ...
  if (!newAudio.duration) {
    const arrayBuffer = await window.flipflip.api.readBinaryFile(audio.url)
...
export async function getLocalPath (cachingDirectory: string, source: string) {
  return await window.flipflip.api.cachePath(cachingDirectory, source, 'local')
}

export async function getCachePath (directory: string, source?: string) {
  return await window.flipflip.api.cachePath(directory, source)
}
...
export function filterSource (
  ...
  if (filter === '<Mergeable>') {
    ...
    const type = getSourceType(source.url)
    if (type === ST.video) {
      const duration = clip ? clip.end - clip.start : source.duration
      if (duration == null) {
        matchesFilter = false
      } else {
        switch (symbol) {
